import { register, initiateOAuth, getOAuthConfig } from '../../services';
...
export async function registerPage(): Promise<string> {
  ...
  return `
    <div class="glass-card max-w-md mx-auto bg-gray-800 p-6 rounded-lg">
      ...
      ${window.location.hostname === 'localhost' && (oauthConfig.google || oauthConfig.github || oauthConfig.fortytwo)
        ? oauthButtons
      : ''}
...
export function attachRegisterListeners() {
  ...
  if (registerForm) {
    registerForm.addEventListener('submit', async (e) => {
      ...
      try {
        const result = await register(username, email, password, displayName, acceptGdpr);
        console.log("RESULT: ", result);
        if (result.smtp_fallback && result.verification_token) {
          showSMTPFallbackNotification(result.verification_token, 'verification');
        }
